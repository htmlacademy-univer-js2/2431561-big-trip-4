...
const createTripPointTemplate = ({point, pointDestination, pointOffers}) => {
  ...
  <div class="event">
    <time class="event__date" datetime="${humanizeDateTime(dateFrom)}">${humanizeShortDate(dateFrom)}</time>
    ...
    <p class="event__time">
      ...
      &mdash;
      <time class="event__end-time" datetime="${humanizeDateTime(dateTo)}">${humanizeTime(dateTo)}</time>
    </p>
...
export default class TripPointView extends AbstractView{
  ...
  constructor({point, pointDestination, pointOffers, onRedactorClick, onFavoriteClick}){
    super();
    this.#point = point;
